...
my %get_point;my %used_point;my %danger_point;
foreach my $ctg(keys %block){
	my %add_point;
			...
			foreach my $d1(@d1){
				foreach my $d2(@d2){
					if($d2-$d1==1){
						...
						$danger_point{$t_row}=$id_scf;
						print "d1\td2\t$t_row\t$id_scf\n";
						last;
					}
				}
			}
	...
	foreach my $bac_id(keys %add_point){
		if(keys $add_point{$bac_id} > 1){
			my $lone1;
			if($lone_1{$bac_id}){
				$lone1=(sort {$a<=>$b} keys $lone_1{$bac_id})[0];
			}else{
				$lone1=(sort {$a<=>$b} keys $add_point{$bac_id})[-1]+1;
			}
			my $keep;my @keep;
			...
			if(@keep>0){
				#$keep=$keep[-1];
				foreach $keep(@keep){
					$get_point{$bac_id}{$keep}=1;$used_point{$keep}=$bac_id;
				}
			}else{
...
foreach my $bac_id(keys %get_point){
	my @tmp=keys $get_point{$bac_id};my $tmp=join "\t",@tmp;
	print "$bac_id\thave get_point\t$tmp\n";
	$ex_point{$bac_id}=1;
}
#foreach my $key(keys %add_longest){
#	if($ex_point{$key}){next;}
#	print STDERR "$key\tno insert point;try to find again\n";
#}
...
foreach my $key(keys %add_longest){
	...
	#$un{$key}=1;
	if($recheck{$key}){
		if($recheck{$key}{"1"} and $recheck{$key}{"2"} or $recheck{$key}{"1"} and $recheck{$key}{"3"}){
			my @pre;my @aft;
			if(!$recheck{$key}{"2"} and $recheck{$key}{"3"}){
				@pre=@{$recheck{$key}{"3"}};print STDERR "$key get 3\n";
			}else{
				@pre=@{$recheck{$key}{"2"}};
			}
			@aft=@{$recheck{$key}{"1"}};

			my $pre=(sort {$a<=>$b} @pre)[-1];my $aft=(sort {$a<=>$b} @aft)[0];
			if($pre>$aft){print STDERR "$key\tunkown assembly error\n";}
			else{
				my $ctg=$BACinCTG{$key};
				my @nn;
				my $sel;
				foreach my $haa(($pre..$aft)){
					my @tmp=@{$block{$ctg}{$haa}};
					if($tmp[6]=~/scaffold/){push @nn,$haa;}
				}
				if(@nn>1){
					print STDERR "$key\tmulti N  select first one\n";
					#$sel=$nn[0];
				}
				#$sel=$nn[0];
				foreach $sel(@nn){
					unless($sel){print STDERR "$key\tmay not include\tpass\tsuggect discard left scaffold\n";next;}
					if($used_point{$sel}){
						print STDERR "point $sel have been used by $used_point{$sel}\n";
					}
					$get_point{$key}{$sel}=1;
					#$used_point{$sel}=$key;
					$used_point_bu{$sel}=$key;
				}
			}
		}
		elsif(keys $recheck{$key} == 1){
			my @tt=keys $recheck{$key};
			my $ctg=$BACinCTG{$key};
			my $sel;
			if($tt[0] == 1){
				my $nn=$recheck{$key}{$tt[0]}[0];
				while($nn>1){
					$nn--;
					unless($block{$ctg}{$nn}){
						print STDERR "$key\tmay on head\tpass\n";last;
					}
					my @tmp=@{$block{$ctg}{$nn}};
					if($tmp[6]=~/scaffold/){$sel=$nn;last;}
				}
				unless($sel){print STDERR "$key\tmay on head\tpass\n";next;}
				if($used_point{$sel}){
					print STDERR "point $sel have been used by $used_point{$sel}\n";
				}
				$get_point{$key}{$sel}=1;
			...
			#123 equal  12/23(12)
			elsif($tt[0] == 2 or $tt[0] == 3){
				my $nn=$recheck{$key}{$tt[0]}[-1];
				while($nn>0){
					$nn++;
					unless($block{$ctg}{$nn}){
						print STDERR "$key\tmay on tail\tpass\n";last;
					}
					my @tmp=@{$block{$ctg}{$nn}};
					if($tmp[6]=~/scaffold/){$sel=$nn;last;}
					if($nn==$num_c){
						print "$key\tno insert point again\tdiscard\n";
						last;
					}
				}
				unless($sel){print STDERR "$key\tmay on tail\tpass\n";next;}
				if($used_point{$sel}){
					print STDERR "point $sel have been used by $used_point{$sel}\n";
				}
				$get_point{$key}{$sel}=1;
	...
	foreach my $sort(sort {$a<=>$b} keys $block{$ctg}){
		...
		if($block{$ctg}{$sort}[4] eq "N"){			
			...
			if($bacSort{$aft_bac}-$bacSort{$pre_bac}<0){
					...
					$out.="$aa[0]\t$st\t$ed\t$num\t$aa[4]\t$aa[5]\t$aa[6]\t$aa[7]\t$aa[8]\n";
					$check_bu=0;
					if($pre[-1]==3){$check_bu=1;}
					$st=$ed+1;$num++;
				...
				if($get_point{$pre_bac}{$sort}){
					if(($pre[-1]==2 && $aft[-1]<$pre[-1]) or (!$bac12{$pre_bac}{"2"} && $aft[-1]<$pre[-1])){
						...
						if($add_longest{$pre_bac}){
							...
							$out.="$ctg\t$st\t$ed\t$num\tN\t$new_pre_gap_len\tscaffold\tno\tna\n";
							$st=$ed+1;$num++;
							...
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\taddc1\n";
							$st=$ed+1;$num++;
							$ed=$st+$new_aft_gap_len-1;
							#print OUT "$ctg\t$st\t$ed\t$num\tN\t$new_aft_gap_len\tscaffold\tno\tna\n";
							$out.="$ctg\t$st\t$ed\t$num\tN\t$new_aft_gap_len\tscaffold\tno\tna\n";
							$st=$ed+1;$num++;
						}else{
							print "=0=";
							my @aa=@{$block{$ctg}{$sort}};
					...
					print "$pre_bac no point same bac\n";
					my @aa=@{$block{$ctg}{$sort}};
					my $len=$aa[2]-$aa[1]+1;
					$ed=$st+$len-1;
					#print OUT "$aa[0]\t$st\t$ed\t$num\t$aa[4]\t$aa[5]\t$aa[6]\t$aa[7]\t$aa[8]\n";
					...
					if($pre[-1] == 1 && $aft[-1] == 1){
						...
						if($bac12{$aft_bac}{"2"}){
							...
							if(!$ex_point{$aft_bac} && $add_longest{$aft_bac}){
								...
								$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd11\n";
								$st=$ed+1;$num++;
								$ed=$st+$new_aft_gap_len-1;
								#print OUT "$ctg\t$st\t$ed\t$num\tN\t$new_aft_gap_len\tscaffold\tno\tna\n";
								$out.="$ctg\t$st\t$ed\t$num\tN\t$new_aft_gap_len\tscaffold\tno\tna\n";
					...
					if($pre[-1] == 2 && $aft[-1] == 2){
						...
						if($bac12{$pre_bac}{"1"}){
							...
							if(!$ex_point{$pre_bac} && $add_longest{$pre_bac}){
								...
								$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd22\n";
								$st=$ed+1;$num++;
								$ed=$st+$new_aft_gap_len-1;
								#print OUT "$ctg\t$st\t$ed\t$num\tN\t$new_aft_gap_len\tscaffold\tno\tna\n";
								$out.="$ctg\t$st\t$ed\t$num\tN\t$new_aft_gap_len\tscaffold\tno\tna\n";
					...
					if($pre[-1] == 2 && $aft[-1] == 1){
						...
						if(!$ex_point{$pre_bac} && $add_longest{$pre_bac}){
							...
							$ed=$st+100-1;
							#print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
							$out.="$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
							$st=$ed+1;$num++;
							#add pre
							my $insert_scf=$add_longest{$pre_bac}[1];
							my $insert_scf_len=$add_longest{$pre_bac}[0];
							...
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd21p\n";
							$st=$ed+1;$num++;
						}
						#10k gap
						$ed=$st+10000-1;
						#print OUT "$ctg\t$st\t$ed\t$num\tN\t10000\tscaffold\tno\tna\n";
						$out.="$ctg\t$st\t$ed\t$num\tN\t10000\tscaffold\tno\tna\n";
						$st=$ed+1;$num++;
						#add aft
						if(!$ex_point{$aft_bac} && $add_longest{$aft_bac}){
							my $insert_scf=$add_longest{$aft_bac}[1];
							...
							$ed=$st+$insert_scf_len-1;
							#print OUT "$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd21\n";
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd21a\n";
							$st=$ed+1;$num++;
							#add 100 gap
							$ed=$st+100-1;
							#print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
					...
					#three the missing type
					#1:the pre miss 1. as we define the bac to be 2-1. the un-neighbor pre BAC would ouput all record belong to the before overlap
					#1: the before record would be 2 but if it is 1, it means the BAC is not missing bu just not show in the overlap but it still 
					#1: can treat as miss. we check this before the loop.
					#2:the before record is 2 then the BAC is missing in overlap which mean the two BAC no overlap deteted
					#3:the aft begin with 2, then it mean the before BAC is not missing but just not show in the overlap and the aft BAC will have
					#3: another insert point.    begin with 1 means the before BAC is missing.
					
					my $flag=0;
					#check the pre_bac can be insert or not
					my $bacmiss_pre=$pre_bac;
					if($pre[-1]==2){
						#can insert
						if(!$ex_point{$bacmiss_pre} && $add_longest{$bacmiss_pre}){
							$flag=1;
							$ed=$st+100-1;
							#print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
							$out.="$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
							$st=$ed+1;$num++;
							my $insert_scf=$add_longest{$bacmiss_pre}[1];
							my $insert_scf_len=$add_longest{$bacmiss_pre}[0];
							$haveadd{$bacmiss_pre}=1;
							$ed=$st+$insert_scf_len-1;
							#print OUT "$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\taddmisspre\n";
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\taddmisspre\n";
							$st=$ed+1;$num++;
						}
					}
					my $miss_bac_num=$bacSort{$aft_bac}-$bacSort{$pre_bac}-1;
					my $cu=1;my $n_len_check=0;
					while($cu<=$miss_bac_num){
						...
						if(!$ex_point{$bac_miss} && $add_longest{$bac_miss}){
							...
							#$ed=$st+100-1;
							if($n_len_check){
								my $nlen=$n_len_check*30000;
								$n_len_check=0;
								$ed=$st+$nlen-1;
								#print OUT "$ctg\t$st\t$ed\t$num\tN\t$nlen\tscaffold\tno\tna\n";
								$out.="$ctg\t$st\t$ed\t$num\tN\t$nlen\tscaffold\tno\tna\n";
								$st=$ed+1;$num++;
							}else{
								$ed=$st+100-1;
								#print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
							...
							$haveadd{$bac_miss}=1;
							$ed=$st+$insert_scf_len-1;
							#print OUT "$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\taddmiss$cu\n";
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\taddmiss$cu\n";
							$st=$ed+1;$num++;
						}
						#may be need to add 30k if the BAC is really miss
						if(!$add_longest{$bac_miss}){
							$n_len_check++;
						}
						$cu++;
					}
					#check the aft_bac can be insert or not
					my $bacmiss=$aft_bac;
					if($aft[-1]==1){
						if(!$ex_point{$bacmiss} && $add_longest{$bacmiss}){
							...
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\taddmissaft\n";
							$st=$ed+1;$num++;
						}
					}
					if($flag){
			...
			#check used_point_bu
			if(($used_point{$sort} and !$haveadd{$used_point{$sort}} and $add_longest{$used_point{$sort}} and $check_bu) or ($danger_point{$sort} and !$haveadd{$danger_point{$sort}} and $add_longest{$danger_point{$sort}})){
				my $bac;
				if($used_point{$sort} and !$haveadd{$used_point{$sort}}){$bac=$used_point{$sort};print "$bac $sort used_point\n";}
				elsif($danger_point{$sort} and !$haveadd{$danger_point{$sort}}){$bac=$danger_point{$sort};print "$bac $sort danger\n";}
				#else{print "no point $bac\n";next;}
				$ed=$st+100-1;
				print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
				$st=$ed+1;$num++;
				...
				$haveadd{$bac}=1;
				print "$bac add in check 1\n";
				$ed=$st+$insert_scf_len-1;
				print OUT "$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd_only1\n";
				$st=$ed+1;$num++;
			}
			elsif(($used_point_bu{$sort} and !$haveadd{$used_point_bu{$sort}} and $add_longest{$used_point_bu{$sort}} and $check_bu) or ($danger_point{$sort} and !$haveadd{$danger_point{$sort}} and $add_longest{$danger_point{$sort}})){
				my $bac;
				if($used_point_bu{$sort} and !$haveadd{$used_point_bu{$sort}}){$bac=$used_point_bu{$sort};}
				elsif($danger_point{$sort} and !$haveadd{$danger_point{$sort}}){$bac=$danger_point{$sort};print "$bac $sort danger\n";}
				$ed=$st+100-1;
				print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
				$st=$ed+1;$num++;
				...
				my $insert_scf_len=$add_longest{$bac}[0];
				$haveadd{$bac}=1;
				print "$bac add in check 1 bu\n";
				$ed=$st+$insert_scf_len-1;
				print OUT "$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd_only1bu\n";
				$st=$ed+1;$num++;
				#$ed=$st+100-1;
				#print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
				#$st=$ed+1;$num++;
			...
			$aa[1]=$st;$aa[2]=$ed;$aa[3]=$num;my $out=join "\t",@aa;
			print OUT "$out\n";
			$st=$ed+1;$num++;
			if($last==$sort){
				#need to add last
				my @tmp=@{$block{$ctg}{$sort}};my $tmp_bac=$tmp[5];$tmp_bac=~s/_\d+$//;
				if($tmp[-1]==2 or $tmp[-1]==3){
