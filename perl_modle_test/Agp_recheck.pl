...
die "perl $0 <agp> <all.fa.len> <rel.lst> <data_dir>" if(@ARGV==0);

my ($agp,$scf_len,$rel,$data_dir)=@ARGV;
my %rel;my %bacSort;
read_rel($rel,\%rel,$data_dir,\%bacSort);
my %scf_len;my %bacTolen;
read_scf_len($scf_len,\%scf_len,\%bacTolen);
my %used;
good($agp,\%used,\%scf_len,\%rel,\%bacTolen,\%bacSort);


sub good{
	my ($agp,$used,$scf_len,$rel,$bacTolen,$bacSort)=@_;

	#firstly check the ovl repeat and see if we can check for the redundant BACs
	#find out repeat
	my $sort=1;
	print "deal $agp\n";print STDERR "deal $agp\n";
	open IN,$agp or die;
		...
		if($count{$scf}>1){
			...
			if(@select>=2){
				...
				if($not_scf{$not[0]} eq $not_scf{$not[1]}){
					...
					if($oo0[-3] eq $oo1[-3]){
						print "problem in cross_link\tcheck hand\t$scf\tdelete short\n";
						#delet the short one
					...
					print "$first[5]\t$len_first\t$last[5]\t$len_last\n";
	...
	my %ex_len;my $row=1;my %all_re;my %check_123;my %block_d;my $sort_d=1;my %row_sort;
	while(<IN>){
		if(/bac/){
	...
	foreach my $row_sort(sort {$a<=>$b} keys %all_re){
		my $record=$all_re{$row_sort};
		if($record=~/bac/ && ($row_sort>1 && $row_sort<$row-1)){
			my $pre=$row_sort-1;my $aft=$row_sort+1;
			if($all_re{$pre}=~/scaffold/ && $all_re{$aft}=~/scaffold/){
				my @aa=split /\s+/,$record;
				if($aa[-1]==1){
					my $ida=$aa[5];$ida=~s/_\d+$//;
	...
	foreach my $bac_id(keys %add_point){
		if(keys $add_point{$bac_id} > 1){
			unless($lone_1{$bac_id} && keys $lone_1{$bac_id} > 0){print "$bac_id\tno lone_1\n";next;}
