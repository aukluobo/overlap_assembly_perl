...
foreach my $key(keys %add_longest){
	...
	#$un{$key}=1;
	if($recheck{$key}){
		if($recheck{$key}{"1"} and $recheck{$key}{"2"}){
			my @pre=@{$recheck{$key}{"2"}};my @aft=@{$recheck{$key}{"1"}};
			...
			if($pre>$aft){print STDERR "$key\tunkown assembly error\n";}
				...
				$get_point{$key}{$sel}=1;$used_point{$sel}=$key;$used_point_bu{$sel}=$key;
			}
		}
...
foreach my $ctg(keys %block){
	...
	foreach my $sort(sort {$a<=>$b} keys $block{$ctg}){
		...
		if($block{$ctg}{$sort}[4] eq "N"){			
				...
				if($get_point{$pre_bac}{$sort}){
					...
					if($pre[-1] == 2 && $aft[-1] == 1){
						...
						if(!$ex_point{$pre_bac} && $add_longest{$pre_bac}){
							...
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd21\n";
							$st=$ed+1;$num++;
						}
						#10k gap
						$ed=$st+10000-1;
					...
					while($cu<=$miss_bac_num){
						...
						if(!$ex_point{$bac_miss} && $add_longest{$bac_miss}){
							...
							$out.="$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\taddmiss\n";
							$st=$ed+1;$num++;
						}
						$cu++;
					}
			...
			#
			#check used_point_bu
			if($used_point_bu{$sort} and !$haveadd{$used_point_bu{$sort}} and $add_longest{$used_point{$sort}} and $check_bu){
				my $bac=$used_point{$sort};
				$ed=$st+100-1;
				print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
				$st=$ed+1;$num++;
				...
				$ed=$st+$insert_scf_len-1;
				print OUT "$ctg\t$st\t$ed\t$num\tW\t$insert_scf\t1\t$insert_scf_len\t+\tadd_only\n";
				$st=$ed+1;$num++;
				#$ed=$st+100-1;
				#print OUT "$ctg\t$st\t$ed\t$num\tN\t100\tscaffold\tno\tna\n";
				#$st=$ed+1;$num++;
